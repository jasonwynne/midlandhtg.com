		...
		// checking to see if they have answered all of the questions by comparing the amount of questions to the order sidebar
			...
			if(myCount-countActive==countAnwsered && myProduct != 'combo'){
					...
					$(orderTypes+' .recommended-product').each(function(){
						...
						if($(this).hasClass(mySqFtNum) && myBrand == brandName.toLowerCase() && myProduct != 'water-heater' ) {
							$(this).removeClass('hidden');				
						}
						if($(this).hasClass('gal'+myGal) && myBrand == brandName.toLowerCase() && myProduct == 'water-heater' ) {
							$(this).removeClass('hidden');	
						}
					});
					
					//addTotalPrice(my-existing);
...
// step 4 finish 
	...
	$('a.btn-finish-step4').click(function(evt){
		...
		if($('input#none-check').prop('checked')==true){
			...
			$('.ao-product-holder .add-on-product').each(function(){
				if($(this).hasClass('selected')){				
					...
					$('#my-add-ons .ao-info').append('<div data-ao-cost="'+aoProductCost+'">'+aoProductBrand+' '+aoProductID+' '+aoProductType+'</div>');
				}	
			});
