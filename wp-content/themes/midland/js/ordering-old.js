	...
	$('a.btn-finish').click(function(evt) {
		...
		if(myStep===3){
			...
			if(numProducts==1) {
				...
				var myProdType = $('.product-num-0').attr('data-product-type').toLowerCase();
				var selectedProd = $('#'+myProdType+'-recommendations .selected');	
				var myProductOrderID = selectedProd.attr('data-product-id');	
				var myProductOrderType = selectedProd.parent().attr('data-product-type');
				var myTotalCost = selectedProd.children('.total-cost').html();
				var myProductBrnd = selectedProd.attr('data-product-brand').toLowerCase();
				$('#order-info-holder .total span').html(myTotalCost);
				$('.product-num-0').append('<div class="prodID">'+myProductOrderID+'</div>');
				
				
				console.debug(myProductOrderID+' '+myProductOrderType+' '+myTotalCost+' '+myProductBrnd);
